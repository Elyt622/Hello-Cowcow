...
object CowCowUnstakePreviewCodec {
  ...
  fun encode(cowNonces: List<String>): Encoded {
    require(cowNonces.isNotEmpty()) { "At least one staked CowCow is required" }

    val normalized = cowNonces.map(::normalizeNonce)
    require(normalized.distinct().size == normalized.size) {
      "CowCow unstake preview contains duplicate nonces"
    }

    val payload = buildString {
      append(CowCowConfig.UNSTAKE_FUNCTION)
